...
describe("Testes da função calcularFaltasTotais()", () => {
  test("Deveria retornar 15 para 5 faltas", () => {
    expect(calcularFaltasTotais(5)).toBe(15);
  });

  test("Deveria lançar a exceção 'Valor inválido' para -1 faltas", () => {
    expect(() => calcularFaltasTotais(-1)).toThrow("Valor inválido");
  });

  test("Deveria lançar a exceção 'Valor inválido' para 21 faltas", () => {
...
describe("Testes da função calcularMedia()", () => {
  ...
  test("Deveria lançar a exceção 'Nota inválida' para nota maior que 10.0", () => {
  ...
  test("Deveria lançar a exceção 'Nota inválida' para nota menor que 0.0", () => {
    expect(() => calcularMedia(-1.0, 10.0)).toThrow("Nota inválida");
  });

  test("Deveria retornar 7.6 para notas 9.75 e 5.5", () => {
    expect(calcularMedia(9.75, 5.5)).toBe(7.6);
  });
});
...
describe("Testes da função avaliarAluno()", () => {
  ...
  test("Deveria retornar 'Reprovado por média' para média 1.5 e faltas totais iguais a 3", () => {
